...
```
## 2. 下载预训练模型
- 下载预训练模型到chapter08/04_YOLOX/tools目录下
- 官方下载地址：https://github.com/Megvii-BaseDetection/YOLOX/releases/download/0.1.1rc0/yolox_s.pth
- 本项目预训练模型存储路径：../../../02_models/pretrained_models/yolox_s.pth
- 本项目已训练模型存储路径：../../../02_models/fgai_trained_models/chapter08/04_YOLOX/last_epoch_ckpt.pth
## 3. fgai-cv-basis运行方式
```
# 数据集配置文件get_yolox_datadir函数：设置COCO目录
chapter08/YOLOX/yolox/data/dataloading.py

# 参数配置文件
chapter08/YOLOX/yolox/exp/yolox_base.py

cd chapter08/04_YOLOX/tools/
# 训练程序，模型文件保存于chapter08/04_YOLOX/tools/YOLOX_outputs/yolox_s/last_epoch_ckpt.pth
python train.py

# 单张推理程序，推理结果保存chapter08/04_YOLOX/tools/YOLOX_outputs/yolox_s/vis_res
python demo.py
```

![alt text](tools/demo.jpg)
