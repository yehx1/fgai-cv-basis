...
## 2. 下载预训练模型
- 如果未下载预训练模型，程序会自动下载。
- 下载预训练模型到chapter08/03_yolov5目录下
- 官方下载地址：https://github.com/ultralytics/yolov5/releases/download/v7.0/yolov5s.pt
- 本项目预训练模型存储路径：../../02_models/pretrained_models/yolov5s.pt
- 本项目已训练模型存储路径：../../02_models/fgai_trained_models/chapter08/03_yolov5/best.pt
## 3. fgai-cv-basis运行方式
```
# 数据集配置文件
chapter08/03_yolov5/data/coco128.yaml

cd chapter08/03_yolov5
# 训练程序，模型文件保存于runs/trian/exp<id>/weights/best.pt
python train.py

# 批量推理程序，推理结果保存于runs/trian/exp<id>/
python detect.py

# 单张推理程序，推理结果保存于demo.txt
python demo.py
```
