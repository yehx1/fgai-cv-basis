...
## 2. 下载预训练模型
- 如果未下载预训练模型，程序会自动下载。
- 下载预训练模型到chapter08/05_YOLOv6/tools/weights目录下
- 官方下载地址：https://github.com/meituan/YOLOv6/releases/download/0.4.0/yolov6s.pt
- 本项目预训练模型存储路径：../../../../02_models/pretrained_models/yolov6s.pt yolov6s.pt
- 本项目已训练模型存储路径：../../../02_models/fgai_trained_models/chapter08/05_YOLOv6/best_ckpt.pt
## 3. fgai-cv-basis运行方式
```
# 数据集配置文件
chapter08/05_YOLOv6/data/coco.yaml

cd chapter08/05_YOLOv6/tools/
# 训练程序，模型文件保存于chapter08/05_YOLOv6/runs/train/exp<id>/weights/best_ckpt.pt
python train.py

# 单张/批量推理程序，推理结果保存chapter08/05_YOLOv6/tools/runs/inference/exp
python infer.py
```
