...
class ComputeLoss:
    ...
    def __call__(
        ...
        if all(feat.shape[2:] == cfsize for feat, cfsize in zip(feats, self.cached_feat_sizes)):
            ...
            self.cached_feat_sizes = [feat.shape[2:] for feat in feats] # [torch.Size([80, 80]), torch.Size([40, 40]), torch.Size([20, 20])]
        ...
        gt_bboxes_scale = torch.tensor([batch_width, batch_height, batch_width, batch_height]).type_as(pred_scores) # tensor([640., 640., 640., 640.], device='cuda:0', dtype=torch.float16)
        batch_size = pred_scores.shape[0]

        # targets [batch_id, cls, x, y, w, h]
        targets =self.preprocess(targets, batch_size, gt_bboxes_scale) # torch.Size([b, k, 5]), [cls, x, y, x, y]
        gt_labels = targets[:, :, :1] # torch.Size([b, k, 1])
        gt_bboxes = targets[:, :, 1:] #xyxy, torch.Size([b, k, 4])
        mask_gt = (gt_bboxes.sum(-1, keepdim=True) > 0).float() # 有效box标记，针对batch>1情况

        # pboxes
        anchor_points_s = anchor_points / stride_tensor # 锚点在特征图上的中心坐标，torch.Size([8400, 2])
        pred_bboxes = self.bbox_decode(anchor_points_s, pred_distri) #xyxy, 每个锚点预测的回归位置，torch.Size([b, 8400, 4])
            ...
            if epoch_num < self.warmup_epoch:
                ...
                target_labels, target_bboxes, target_scores, fg_mask = \
                    self.formal_assigner(
                        ...
                        gt_labels,
                        gt_bboxes,
                        mask_gt) # 根据中心点、IOU、simOTA来确定前景点及其target
        ...
        # rescale bbox
        target_bboxes /= stride_tensor # torch.Size([b, 8400, 4])

        # cls loss
        target_labels = torch.where(fg_mask > 0, target_labels, torch.full_like(target_labels, self.num_classes)) # torch.Size([b, 8400])
        one_hot_label = F.one_hot(target_labels.long(), self.num_classes + 1)[..., :-1] # torch.Size([b, 8400, 80])
        loss_cls = self.varifocal_loss(pred_scores, target_scores, one_hot_label) # Varifocal Loss
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points,
        ...
        num_pos = fg_mask.sum() # n
        if num_pos > 0:
            # iou loss
            bbox_mask = fg_mask.unsqueeze(-1).repeat([1, 1, 4]) # torch.Size([b, 8400, 4])
            pred_bboxes_pos = torch.masked_select(pred_bboxes,
                                                  bbox_mask).reshape([-1, 4]) # torch.Size([n, 4])
            target_bboxes_pos = torch.masked_select(
                target_bboxes, bbox_mask).reshape([-1, 4]) # torch.Size([n, 4])
            bbox_weight = torch.masked_select(
                target_scores.sum(-1), fg_mask).unsqueeze(-1) # torch.Size([n, 1])
            loss_iou = self.iou_loss(pred_bboxes_pos,
                                     target_bboxes_pos) * bbox_weight # torch.Size([n, 1])
            if target_scores_sum > 1:
