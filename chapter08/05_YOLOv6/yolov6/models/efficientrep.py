...
class EfficientRep(nn.Module):
    ...
    def forward(self, x): # torch.Size([b, 3, 640, 640])

        outputs = []
        x = self.stem(x) # RepVGGBlock, torch.Size([b, 32, 320, 320])
        x = self.ERBlock_2(x) # ERBlock，多个RepVGGBlock，torch.Size([b, 64, 160, 160])
        if self.fuse_P2: # 下降2**2=4倍
            outputs.append(x)
        x = self.ERBlock_3(x) # ERBlock，多个RepVGGBlock，torch.Size([b, 128, 80, 80])
        outputs.append(x)
        x = self.ERBlock_4(x) # ERBlock，多个RepVGGBlock，torch.Size([b, 256, 40, 40])
        outputs.append(x)
        x = self.ERBlock_5(x) # ERBlock，多个RepVGGBlock+SPPF，torch.Size([b, 512, 20, 20])
        outputs.append(x)
        # [torch.Size([b, 64, 160, 160]), torch.Size([b, 128, 80, 80]), torch.Size([b, 256, 40, 40]), torch.Size([b, 512, 20, 20])]
