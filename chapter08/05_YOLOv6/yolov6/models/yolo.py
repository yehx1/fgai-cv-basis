...
class Model(nn.Module):
    ...
    def forward(self, x): # torch.Size([b, 3, 640, 640])
        ...
        x = self.backbone(x) # [torch.Size([b, 64, 160, 160]), torch.Size([b, 128, 80, 80]), torch.Size([b, 256, 40, 40]), torch.Size([b, 512, 20, 20])]
        x = self.neck(x) # PAN(RepVGGBlock)特征融合，[torch.Size([b, 64, 80, 80]), torch.Size([b, 128, 40, 40]), torch.Size([b, 256, 20, 20])]
        if not export_mode:
            featmaps = []
            featmaps.extend(x)
        x = self.detect(x) # [# torch.Size([b, 8400, 80]), torch.Size([b, 8400, 80])]
