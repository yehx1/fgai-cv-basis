...
class Detect(nn.Module):
    ...
    def forward(self, x): # [torch.Size([b, 64, 80, 80]), torch.Size([b, 128, 40, 40]), torch.Size([b, 256, 20, 20])]
        if self.training:
            cls_score_list = []
            reg_distri_list = []

            for i in range(self.nl):
                x[i] = self.stems[i](x[i]) # 1x1卷积、BN、SiLU，torch.Size([b, 64, 80, 80])
                cls_x = x[i] # torch.Size([b, 64, 80, 80])
                reg_x = x[i] # torch.Size([b, 64, 80, 80])
                cls_feat = self.cls_convs[i](cls_x) # 3x3卷积、BN、SiLU，torch.Size([b, 64, 80, 80])
                cls_output = self.cls_preds[i](cls_feat) # 1x1卷积, torch.Size([b, 80, 80, 80])
                reg_feat = self.reg_convs[i](reg_x) # 3x3卷积、BN、SiLU，torch.Size([b, 64, 80, 80])
                reg_output = self.reg_preds[i](reg_feat) # 1x1卷积, torch.Size([b, 4, 80, 80])

                cls_output = torch.sigmoid(cls_output) # 转为概率， torch.Size([b, 80, 80, 80])
                cls_score_list.append(cls_output.flatten(2).permute((0, 2, 1))) # torch.Size([b, 6400, 80])
                reg_distri_list.append(reg_output.flatten(2).permute((0, 2, 1))) # torch.Size([b, 6400, 4])

            cls_score_list = torch.cat(cls_score_list, axis=1) # torch.Size([b, 8400, 80])
            reg_distri_list = torch.cat(reg_distri_list, axis=1) # torch.Size([b, 8400, 4])
