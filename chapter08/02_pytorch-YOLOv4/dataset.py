...
class Yolo_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bboxes = np.array(self.truth.get(img_path), dtype=np.float_)
        ...
        for i in range(use_mixup + 1):
            ...
            if img is None:
                continue
            oh, ow, oc = img.shape
            dh, dw, dc = np.array(np.array([oh, ow, oc]) * self.cfg.jitter, dtype=np.int_)
    ...
    def _get_val_item(self, index):
        ...
        """
        img_path = self.imgs[index]
        bboxes_with_cls_id = np.array(self.truth.get(img_path), dtype=np.float_)
