...
## 2. 下载预训练模型
下载预训练模型到chapter08/02_pytorch-YOLOv4/checkpoints目录下
### 2.1. 原始项目预训练模型下载地址：
（1）darknet
...
### 2.2. 本项目预训练模型存储路径
- ../../../02_models/pretrained_models/yolov4.pth yolov4.pth
- ../../../02_models/pretrained_models/yolov4.conv.137.pth yolov4.conv.137.pth
### 2.3. 本项目已训练模型存储路径
- ../../02_models/fgai_trained_models/chapter08/02_pytorch-YOLOv4/yolov4_epoch_last.pth
## 3. fgai-cv-basis运行方式
```
# 训练等参数配置文件
chapter08/02_pytorch-YOLOv4/cfg.py

cd chapter08/02_pytorch-YOLOv4
# 训练程序，模型文件保存于checkpoints/yolov4_epoch_last.pth
python train.py

# 推理程序，推理结果保存于chapter08/02_pytorch-YOLOv4/predictions.jpg
python models.py
```
