...
## 2. 下载预训练模型
- 如果未下载预训练模型，程序会自动下载。
- 下载预训练模型到chapter08/08_yolov9目录下
- 官方下载地址：https://github.com/ultralytics/assets/releases/download/v8.3.0/yolov9s.pt
- 本项目预训练模型存储路径：../../02_models/pretrained_models/yolov9s.pt
- 本项目已训练模型存储路径：../../02_models/fgai_trained_models/chapter08/08_yolov9/best.pt
## 3. fgai-cv-basis运行方式
```
# 数据集配置文件
chapter08/08_yolov9/data/coco.yaml

cd chapter08/08_yolov9
# 训练程序，模型文件保存于chapter08/08_yolov9/runs/train/exp<id>/weights/best.pt
python train_dual.py

# 单张/批量推理程序，推理结果保存于chapter08/08_yolov9/runs/detect/exp<id>
