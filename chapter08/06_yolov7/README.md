...
```
## 2. 下载预训练模型
- 下载预训练模型到chapter08/06_yolov7目录下
- 官方下载地址：https://github.com/WongKinYiu/yolov7/releases/download/v0.1/yolov7.pt
- 本项目预训练模型存储路径：../../02_models/pretrained_models/yolov7.pt
- 本项目已训练模型存储路径：../../02_models/fgai_trained_models/chapter08/06_yolov7/best.pt
## 3. fgai-cv-basis运行方式
```
# 数据集配置文件
chapter08/06_yolov7/data/coco.yaml

cd chapter08/06_yolov7/
# 训练程序，模型文件保存于chapter08/06_yolov7/runs/train/exp<id>/weights/best.pt
python train.py

# 单张/批量推理程序，推理结果保存chapter08/06_yolov7/runs/detect/exp<id>
python detect.py
```
