...
if __name__ == '__main__':
    ...
    extract_package(os.path.join(save_dir, 'model_package_with_config.zip'), extract_to)
    ...
    # 加载并解密模型
    decrypted_model = load_and_decrypt_model(os.path.join(extract_to, "encrypted_model.bin"))
    ...
    # 设置置信度阈值和NMS阈值
    confidence_threshold = loaded_config['threshold']
    nms_threshold = 0.4
    ...
    # 保存处理后的图像
    cv2.imwrite(f'{save_dir}/onnx_unzip_infer.jpg', image)
