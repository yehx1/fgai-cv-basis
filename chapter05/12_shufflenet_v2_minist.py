...
# 定义 ShuffleNetV2 模型，修改输入通道和全连接层
class ShuffleNetV2(nn.Module):
    def __init__(self, num_classes=10):
        super(ShuffleNetV2, self).__init__()
        # 使用预训练的 ShuffleNetV2 模型，并进行自定义修改
        self.shufflenet = models.shufflenet_v2_x1_0(pretrained=True)
        
        # 修改第一个卷积层输入通道为1（因为 MNIST 是单通道灰度图像）
        self.shufflenet.conv1[0] = nn.Conv2d(1, 24, kernel_size=3, stride=2, padding=1)
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # ShuffleNetV2 需要输入 224x224 的图像
        transforms.ToTensor(),
    ...
    # 保存模型
    torch.save(model.state_dict(), f'{save_dir}/shufflenet_mnist.pth')
    print(f'model save path: {save_dir}/shufflenet_mnist.pth')

    # 绘制并保存损失和准确率曲线
    plot_metrics(train_losses, accuracies, f'{save_dir}/shufflenet_loss_accuracy_curve.png')
