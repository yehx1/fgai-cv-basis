...
# 定义 VGG19 模型，修改输入通道和全连接层
class VGG19(nn.Module):
    def __init__(self, num_classes=10):
        super(VGG19, self).__init__()
        # 使用预训练的 VGG19 模型，并进行自定义修改
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # VGG19 需要输入 224x224 的图像
        transforms.ToTensor(),
    ...
    # 保存模型
    torch.save(model.state_dict(), f'{save_dir}/vgg19_mnist.pth')
    print(f'model save path: {save_dir}/vgg19_mnist.pth')

    # 绘制并保存损失和准确率曲线
    plot_metrics(train_losses, accuracies, f'{save_dir}/vgg19_loss_accuracy_curve.png')
