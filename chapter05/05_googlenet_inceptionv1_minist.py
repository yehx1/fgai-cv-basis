...
# 定义 Inception v1 模型，修改输入通道和全连接层
class InceptionV1(nn.Module):
    def __init__(self, num_classes=10):
        super(InceptionV1, self).__init__()
        # 使用预训练的 Inception v1 模型
...
if __name__ == '__main__':
    ...
    model = InceptionV1(num_classes=10).to(device)
    ...
    # 保存模型
    torch.save(model.state_dict(), f'{save_dir}/inceptionv1_mnist.pth')
    print(f'model save path: {save_dir}/inceptionv1_mnist.pth')

    # 绘制并保存损失和准确率曲线
    plot_metrics(train_losses, accuracies, f'{save_dir}/inceptionv1_loss_accuracy_curve.png')