...
import pretrainedmodels  # 用于加载 Inception v4 模型
import matplotlib.pyplot as plt

# 定义 Inception v4 模型，修改输入通道和全连接层
class InceptionV4(nn.Module):
    def __init__(self, num_classes=10):
        super(InceptionV4, self).__init__()
        # 使用预训练的 Inception v4 模型
        self.inception = pretrainedmodels.__dict__['inceptionv4'](pretrained='imagenet')
        
        # 修改分类器（全连接层）以适应 MNIST 的 10 类分类
        self.inception.last_linear = nn.Linear(self.inception.last_linear.in_features, num_classes)
...
if __name__ == '__main__':
    ...
    # 修改数据预处理，将灰度图像复制到3个通道
    transform = transforms.Compose([
        transforms.Resize((299, 299)),  # Inception v4 使用 299x299 输入大小
    ...
    # 保存模型
    torch.save(model.state_dict(), f'{save_dir}/inceptionv4_mnist.pth')
    print(f'model save path: {save_dir}/inceptionv4_mnist.pth')

    # 绘制并保存损失和准确率曲线
    plot_metrics(train_losses, accuracies, f'{save_dir}/inceptionv4_loss_accuracy_curve.png')
