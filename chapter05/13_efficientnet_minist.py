...
# 定义 EfficientNet 模型，修改输入通道和全连接层
class EfficientNet(nn.Module):
    def __init__(self, num_classes=10):
        super(EfficientNet, self).__init__()
        # 使用预训练的 EfficientNet_B0 模型，并进行自定义修改
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # EfficientNet 需要输入 224x224 的图像
        transforms.ToTensor(),
    ...
    torch.save(model.state_dict(), f'{save_dir}/efficientnet_mnist.pth')
    print(f'model save path: {save_dir}/efficientnet_mnist.pth')

    plot_metrics(train_losses, accuracies, f'{save_dir}/efficientnet_loss_accuracy_curve.png')
