...
# 定义 DenseNet-121 模型，修改输入通道和全连接层
class DenseNet121(nn.Module):
    def __init__(self, num_classes=10):
        super(DenseNet121, self).__init__()
        # 使用预训练的 DenseNet-121 模型，并进行自定义修改
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # DenseNet121 需要输入 224x224 的图像
        transforms.ToTensor(),
    ...
    torch.save(model.state_dict(), f'{save_dir}/densenet121_mnist.pth')
    print(f'model save path: {save_dir}/densenet121_mnist.pth')

    plot_metrics(train_losses, accuracies, f'{save_dir}/densenet121_loss_accuracy_curve.png')
