...
class ResNet(nn.Module):
    ...
    def forward(self, x): # torch.Size([b, 3, 448, 448])
        x = self.conv1(x) # Conv2d(3, 64, (7, 7), (2, 2), (3, 3)), torch.Size([b, 64, 224, 224])
        x = self.bn1(x) #
        x = self.relu(x)
        x = self.maxpool(x) # torch.Size([b, 64, 112, 112])

        x = self.layer1(x) # torch.Size([b, 256, 112, 112])
        x = self.layer2(x) # torch.Size([b, 512, 56, 56])
        x = self.layer3(x) # torch.Size([b, 1024, 28, 28])
        x = self.layer4(x) # torch.Size([b, 2048, 14, 14])
        x = self.layer5(x) # torch.Size([b, 256, 14, 14])
        # x = self.avgpool(x)
        # x = x.view(x.size(0), -1)
        # x = self.fc(x)
        x = self.conv_end(x) # torch.Size([b, 30, 14, 14])
        x = self.bn_end(x)
        x = F.sigmoid(x) #归一化到0-1
        # x = x.view(-1,7,7,30)
        x = x.permute(0,2,3,1) # torch.Size([b, 14, 14, 30])
