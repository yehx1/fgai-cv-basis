...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            classification = classifications[j, :, :] # torch.Size([99765, 80])
            regression = regressions[j, :, :] # torch.Size([99765, 4])

            bbox_annotation = annotations[j, :, :] # torch.Size([K, 5]), (x1, y1, x2, y2, cls)
            bbox_annotation = bbox_annotation[bbox_annotation[:, 4] != -1] # 去除batch内补全部分
            ...
            targets = torch.ones(classification.shape) * -1 # torch.Size([99765, 80])
            ...
            if torch.cuda.is_available():
                alpha_factor = torch.ones(targets.shape).cuda() * alpha # torch.Size([99765, 80])
            else:
                alpha_factor = torch.ones(targets.shape) * alpha

            alpha_factor = torch.where(torch.eq(targets, 1.), alpha_factor, 1. - alpha_factor) # 正样本为alpha，其它为1-alpha
            focal_weight = torch.where(torch.eq(targets, 1.), 1. - classification, classification)
            focal_weight = alpha_factor * torch.pow(focal_weight, gamma) # torch.Size([99765, 80])
            ...
            if torch.cuda.is_available():# 中间样本损失设置为0，即不考虑中间样本
            ...
            if positive_indices.sum() > 0: # 转为偏移标签
                assigned_annotations = assigned_annotations[positive_indices, :] # torch.Size([N, 5])
