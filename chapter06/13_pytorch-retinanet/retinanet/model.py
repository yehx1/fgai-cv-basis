...
class PyramidFeatures(nn.Module):
    ...
    def forward(self, inputs):
        ...
        P7_x = self.P7_1(P6_x) # ReLU(), torch.Size([b, 256, 10, 13])
...
class RegressionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv4(out)
        out = self.act4(out) 
        # 使用5个3x3卷积获取回归预测结果，特征图每个位置预测9个anchor，每个anchor有4个回归参数，
        out = self.output(out) # torch.Size([b, 36, h, w])

        # out is B x C x W x H, with C = 4*num_anchors
        out = out.permute(0, 2, 3, 1)

        return out.contiguous().view(out.shape[0], -1, 4) # torch.Size([b, hxw, 36])
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            img_batch = inputs
        # img_batch: torch.Size([b, 3, 640, 832])
        x = self.conv1(img_batch) # Conv2d(3, 64, (7, 7), (2, 2), (3, 3)), torch.Size([b, 64, 320, 416])
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) # torch.Size([b, 64, 160, 208])

        x1 = self.layer1(x) # x1: Bottleneck Block, torch.Size([b, 256, 160, 208])
        x2 = self.layer2(x1) # x2: Bottleneck Block, torch.Size([b, 512, 80, 104])
        x3 = self.layer3(x2) # x3: Bottleneck Block, torch.Size([b, 1024, 40, 52])
        x4 = self.layer4(x3) # x4: Bottleneck Block, torch.Size([b, 2048, 20, 26])
        # torch.Size([b, 256, 80, 104]), torch.Size([b, 256, 40, 52]), torch.Size([b, 256, 20, 26]), torch.Size([b, 256, 10, 13]), torch.Size([b, 256, 5, 7])
        features = self.fpn([x2, x3, x4])
        # 使用5个3x3卷积获取回归预测结果，特征图每个位置预测9个anchor，每个anchor有4个回归参数，torch.Size([b, hxwx9, 4])
        regression = torch.cat([self.regressionModel(feature) for feature in features], dim=1) # 11085, torch.Size([b, 99765, 4])
        # 使用5个3x3卷积获取回归预测结果，特征图每个位置预测9个anchor，每个anchor预测的数量为类别数量num_cls=80，torch.Size([b, hxwx9, 80])
        classification = torch.cat([self.classificationModel(feature) for feature in features], dim=1) # 11085, torch.Size([b, 99765, 80])

        anchors = self.anchors(img_batch) # torch.Size([b, 99765, 4])
