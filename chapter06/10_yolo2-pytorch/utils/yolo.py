...
def _offset_boxes(boxes, im_shape, scale, offs, flip):
    if len(boxes) == 0:
        return boxes
    boxes = np.asarray(boxes, dtype=np.float_)
    boxes *= scale
    boxes[:, 0::2] -= offs[0]
    boxes[:, 1::2] -= offs[1]
...
def preprocess_train(data, size_index):
    ...
    # im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    # im /= 255
    boxes = np.asarray(boxes, dtype=np.int_)
...
def postprocess(bbox_pred, iou_pred, prob_pred, im_shape, cfg, thresh=0.05,
    ...
    bbox_pred = yolo_to_bbox(
        np.ascontiguousarray(bbox_pred, dtype=np.float_),
        np.ascontiguousarray(anchors, dtype=np.float_),
        H, W)
    bbox_pred = np.reshape(bbox_pred, [-1, 4])
    bbox_pred[:, 0::2] *= float(im_shape[1])
    bbox_pred[:, 1::2] *= float(im_shape[0])
    bbox_pred = bbox_pred.numpy()
    bbox_pred = bbox_pred.astype(np.int_)
    ...
    # NMS
    keep = np.zeros(len(bbox_pred), dtype=np.int_)
...
def _bbox_targets_perimage(im_shape, gt_boxes, cls_inds, dontcare_areas, cfg):
    ...
    # anchors = cfg.anchors
    H, W = cfg.out_size
    gt_boxes = np.asarray(gt_boxes, dtype=np.float_)
    # TODO: dontcare areas
    dontcare_areas = np.asarray(dontcare_areas, dtype=np.float_)
