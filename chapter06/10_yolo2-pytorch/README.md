...
```
## 2. 下载预训练模型
- darknet19.weights.npz：官方下载地址为[darknet19.weights.npz)](https://drive.google.com/file/d/0B4pXCfnYmG1WRG52enNpcV80aDg/view?usp=sharing&resourcekey=0-LUq4HFw9tSLKqMcPZeWsaQ)
- 本项目预训练模型存储路径：../../02_models/pretrained_models/darknet19.weights.npz
- 本项目已训练模型存储路径：../../02_models/fgai_trained_models/chapter06/10_yolo2-pytorch/darknet19_voc07trainval_exp3_final.h5。
## 3. fgai-cv-basis运行方式
```
# 数据集等配置文件
chapter06/10_yolo2-pytorch/cfgs/config.py

cd chapter06/10_yolo2-pytorch
# 复制预训练模型
cp ../../02_models/pretrained_models/darknet19.weights.npz models/

# 训练程序，模型文件保存于models/training/darknet19_voc07trainval_exp3/darknet19_voc07trainval_exp3_final.h5
python train.py

# 推理程序，推理结果保存于chapter06/10_yolo2-pytorch/result/
python demo.py
```
![alt text](demo.jpg)
