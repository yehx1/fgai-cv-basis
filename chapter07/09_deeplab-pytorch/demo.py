...
def preprocessing(image, device, CONFIG):
    ...
    # 计算需要填充的尺寸
    pad_bottom = desired_size - height  # 仅填充下方
    pad_right = desired_size - width    # 仅填充右侧

    # 填充图像
    image = cv2.copyMakeBorder(
        image,
        top=0, bottom=pad_bottom,  # 只填充下方
        left=0, right=pad_right,  # 只填充右侧
        borderType=cv2.BORDER_CONSTANT,
        value=(0, 0, 0)  # 填充颜色，默认为黑色
    )
...
@click.option(
    ...
    default="data/models/voc12/deeplabv2_resnet101_msc/train/checkpoint_final.pth",
...
@click.option(
    ...
    "--image-path",
    type=click.Path(exists=True),
    default="../../01_data/VOC/VOCdevkit/VOC2007/JPEGImages/000032.jpg",
...
def single(config_path, model_path, image_path, cuda, crf):
    ...
    output_file = "./result/output_image.png"  # Change this to your desired file path
...
@click.option(
    ...
    "--model-path",
    type=click.Path(exists=True),
    default="deeplabv2_resnet101_msc-vocaug-20000.pth",
    help="PyTorch model to be loaded",
)
@click.option(
    "--cuda/--cpu", default=True, help="Enable CUDA if available [default: --cuda]"
...
if __name__ == "__main__":
    import sys
    sys.argv.append('single')  # 添加默认子命令
    main()
