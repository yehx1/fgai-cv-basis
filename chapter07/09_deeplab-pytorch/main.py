...
def test(config_path, model_path, cuda):
    ...
    for image_ids, images, gt_labels in tqdm(
        ...
        output = np.array(labels[0].cpu(), dtype=np.uint8)
        filename = os.path.join(logit_dir, image_ids[0] + "_pred.jpg")
        cv2.imwrite(filename, colors[output])
