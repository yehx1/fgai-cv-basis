...
num_save_iters = 1000
num_print_iters = 100
init_model_path = f"{current_directory}/data/deeplab_largeFOV.pth"
log_path = f"{current_directory}/./exp/log.txt"
model_path_save = f"{current_directory}/./exp/model_last.pth"
root_dir_path = f"{current_directory}/../../01_data/VOC/VOCdevkit/VOC2007"
pred_dir_path = f"{current_directory}/exp/labels/"
...
def train():
    ...
    if os.path.exists(model_path_save):
        model.load_state_dict(torch.load(model_path_save))
    else:
    ...
    train_loader = torch.utils.data.DataLoader(
        VOCDataset(root_dir_path, split='train', crop_size=321, is_scale=False, is_flip=True),
...
def test(model_path_test, use_crf):
    ...
    val_loader = torch.utils.data.DataLoader(
        VOCDataset(root_dir_path, split='train', crop_size=crop_size, label_dir_path='SegmentationClass', is_scale=False, is_flip=False),
        batch_size=batch_size,
        shuffle=False,
        num_workers=4,
        drop_last=False
    ...
    for iter_id, batch in tqdm(enumerate(val_loader)):
        image_ids, images, labels_raw = batch
        images = images.to(device)
        labels = losses.resize_labels(labels_raw, size=(crop_size, crop_size)).to(device)
        logits = model(images)
        ...
        for i in range(len(image_ids)):
            if is_post_process:
                ...
                cv2.imwrite(pred_dir_path + image_ids[i] + '_pred_crf.jpg', colors[output])
            output = np.array(outputs[i].cpu(), dtype=np.uint8)
            cv2.imwrite(pred_dir_path + image_ids[i] + '_pred.jpg', colors[output])
            label_np = np.array(labels_raw[i]).astype(np.uint8)
            cv2.imwrite(pred_dir_path + image_ids[i] + '_gt.jpg', colors[label_np])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path_test', default='./exp/model_last.pth', help='test model path')
    parser.add_argument('--use_crf', default=True, action='store_true', help='use crf or not')
